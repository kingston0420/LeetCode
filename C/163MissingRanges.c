 ...
 */
char* formatRange(int lower, int upper) {
    char* output = (char*)malloc(25*sizeof(char));
    if (lower == upper) 
        sprintf(output, "%d", lower);
    else 
        sprintf(output, "%d->%d", lower, upper);
    return output;
...
char ** findMissingRanges(int* nums, int numsSize, int lower, int upper, int* returnSize){
    ...
    *returnSize = 0;
    ...
    for (int i=0; i<=numsSize; i++) {
        int cur = (i<numsSize) ? nums[i] : upper+1;
        if (prev+1<=cur-1) {
            output[(*returnSize)] = formatRange(prev+1,cur-1);
            (*returnSize) += 1;
        }
        prev = cur;
    }
